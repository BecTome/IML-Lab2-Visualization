...
# %% [markdown]
# In this case, the analysis is done in the Vote dataset. The dataset is loaded 
# and the PCA algorithm is applied to the dataset. The results are shown in the
# following figures.
# 
# All the features are categorical so no previous standardization is needed. 
# Inside the algorithm, data centralization is done.
# 
# To check and analyze the results, all components are preserved even if only 
# the first two are shown in the figures.
